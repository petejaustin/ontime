...
/// * `_player` - Boolean player who wants to reach (0 or 1)
/// * `target` - target set)
/// 
/// # Returns
...
mod tests {
    ...
    fn test_two_state_reachability() {
        ...
        // Only state 1 is the target
        let target = vec![false, true];
        // At time 5, state 0 can reach state 1 (x >= 5)
        let result_5 = reachable_at(&graph, 5, true, &target);
        assert_eq!(result_5, vec![true, true]);
        // At time 4, state 0 cannot reach state 1 (x >= 5 fails)
        let result_4 = reachable_at(&graph, 4, true, &target);
        assert_eq!(result_4, vec![false, true]);
    }
}